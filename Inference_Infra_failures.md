...
## Inference Infrastructure Optimization Analysis

### vLLM Integration Assessment (8GB GPU Constraints)
**Current System**: Custom memory-optimized batching with 6-8GB peak memory per model
- Dynamic batch sizing: 3B models (3 parallel questions), 7B models (2 parallel questions)
- Aggressive memory management: 10% safety margin targeting 6-7GB utilization (73-85%)
- Model-specific optimization with CUDA cleanup between batches

**vLLM Analysis**: ❌ **Not beneficial for 8GB constraints**
```python
# vLLM Memory Overhead
model_memory = 6-8GB                    # Base model
vllm_overhead = 1-2GB                   # KV cache pool + serving infrastructure
total_required = 8-10GB                 # Exceeds 8GB capacity
```
**Root Cause**: vLLM's PagedAttention pre-allocates large KV cache blocks incompatible with memory-constrained environments. Current system's dynamic batching provides superior memory efficiency.

### LMCache Integration Assessment (Adversarial Evaluation)
**Workflow Analysis**: Cross-attack processing on modified image variants
```bash
# Directory Structure (Same filename, different content)
data/test_extracted/dashboard/0001.png      # Original image
data/test_BB_pgd/dashboard/0001.png        # Adversarially perturbed pixels
data/test_BB_fgsm/dashboard/0001.png       # Different adversarial perturbation
data/test_BB_cw_l2/dashboard/0001.png      # Another variant
```

**LMCache Analysis**: ❌ **No performance benefit for adversarial evaluation**
```python
# Expected Cache Behavior
Query                    mm_hash      KV hits    Hit rate
Original 0001.png       abc123...    0          0%
PGD 0001.png           def456...    0          0%  # Different pixel values
FGSM 0001.png          ghi789...    0          0%  # Different perturbations  
CW-L2 0001.png         jkl012...    0          0%  # Unique adversarial noise
```
**Root Cause**: Adversarial attacks modify pixel values creating unique mm_hashes per variant. Vision encoder must re-process each perturbed image, eliminating caching benefits. Cache hit rate ≈ 0% across 17 attack variants.

**Conclusion**: Current memory-optimized architecture optimal for adversarial robustness evaluation workflows.
