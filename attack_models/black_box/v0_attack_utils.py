...
- apply_standardized_ssim_constraint: Apply standardized SSIM constraint (same as transfer attacks)
...
def get_output_path(input_path, attack_type):
    ...
    # Research paper standard: clean/ vs adversarial/blackbox/ for query-based attacks
    output_dir = input_dir.replace('clean', f'adversarial/blackbox/{dir_suffix}')
...
def print_attack_info(output_path, original_image, adv_image, attack_type):
    ...
    print(f"   To:     img_path = 'data/adversarial/blackbox/{attack_type}/' + data['image']")
...
def apply_standardized_ssim_constraint(original_image, adv_image, ssim_threshold=0.85):
    """Apply standardized SSIM constraint using the same binary search logic as transfer attacks
    
    This function ensures all attacks (transfer and black-box) use the same SSIM constraint
    implementation for consistent and fair comparison.
