...
- apply_correct_ssim_constraint: Apply SSIM constraint using correct binary search logic
"""
...
def preprocess_image_for_attack(image, return_tensor=False):
    ...
    img_tensor = transform(image).unsqueeze(0).float()  # Ensure float32 dtype
...
def get_output_path(input_path, attack_type):
    ...
    # Research paper standard: clean/ vs adversarial/whitebox/ for transfer attacks
    output_dir = input_dir.replace('clean', f'adversarial/whitebox/{dir_suffix}')
...
def print_attack_info(output_path, original_image, adv_image, attack_type):
    ...
    print(f"   To:     img_path = 'data/adversarial/whitebox/{attack_type}/' + data['image']")
...
def apply_correct_ssim_constraint(original_image, adv_image, ssim_threshold=0.85):
    """Apply SSIM constraint using correct binary search logic (copied from working black-box attacks)
    
    This function targets an exact SSIM value rather than maximizing SSIM above a threshold.
    It uses the working binary search logic from black-box attacks (v10_square_attack.py).
    
    Args:
        original_image (numpy.ndarray): Original image
        adv_image (numpy.ndarray): Adversarial image
        ssim_threshold (float): Target SSIM value (default: 0.85)
        
    Returns:
        numpy.ndarray: Constrained adversarial image with SSIM ≈ ssim_threshold
    """
    ...
    if abs(current_ssim - target_ssim) > 0.01:  # If more than 0.01 away from target
        ...
        for i in range(20):  # 20 binary search steps for precision
            ...
            # CORRECT LOGIC: Adjust search based on target
        ...
        final_ssim = calculate_ssim(original_image, adv_image)
