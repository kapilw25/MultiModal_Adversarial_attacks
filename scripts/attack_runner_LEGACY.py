...
class AttackOrchestrator:
    ...
    def __init__(self, config: AttackConfig):
        ...
        # Database-only logging (no file dependencies)
    ...
    def extract_metrics_from_log(self, log_content: str) -> Dict[str, Any]:
        ...
        # Extract SSIM - get the FINAL result, not intermediate binary search values
        # First try to get the final "Achieved SSIM" after "Attack: FGSM completed successfully"
        final_result_match = re.search(r'Attack: FGSM completed successfully.*?Achieved SSIM:\s*([0-9.-]+)', log_content, re.DOTALL)
        if final_result_match:
            metrics['ssim_actual'] = float(final_result_match.group(1))
        else:
            # Try to get "Best SSIM" from binary search completion
            best_ssim_match = re.search(r'Best SSIM:\s*([0-9.-]+)', log_content)
            if best_ssim_match:
                metrics['ssim_actual'] = float(best_ssim_match.group(1))
            else:
                # Final fallback to last occurrence
                ssim_matches = re.findall(r'Achieved SSIM[^:]*:\s*([0-9.-]+)', log_content)
                if ssim_matches:
                    metrics['ssim_actual'] = float(ssim_matches[-1])  # Use LAST occurrence
    ...
    def run_whitebox_attack(self, image_path: str, attack_type: str,
        ...
        # Use provided severity level or fall back to config default
        ...
        optimization_method = "direct_epsilon"  # Direct epsilon control
        cmd = [
        ...
        try:
            ...
            for line in iter(process.stdout.readline, ''):
                ...
                if line:
                    ...
                    # Database-only logging (no file operations)
            ...
            # Check for actual SSIM target achievement, not just process success
            process_success = result.returncode == 0 and not any(
                keyword in result.stdout.lower()
                for keyword in ['failed', 'error', 'exception']
            )

            # Real success means achieving target SSIM range - check for multiple success indicators
            ssim_target_achieved = (
                "In Target Range: True" in result.stdout or
                "✅ SUCCESS! SSIM in target range!" in result.stdout or
                "✅ SUCCESS! SSIM in acceptable range" in result.stdout
            )
            success = process_success and ssim_target_achieved
            ...
            # Database-only logging (detailed information stored in database)
    ...
    def run_blackbox_attack(self, image_path: str, attack_type: str,
                          attack_num: int, total_attacks: int, severity_level: str = None, trial_number: int = 1) -> bool:
    ...
    def save_attack_results(self, attack_name: str, image_path: str,
        ...
        execution_data = {
            ...
            'parameters': {
                ...
                'ssim_actual': metrics.get('ssim_actual', 0.0),
        ...
        logger.info(f"[{attack_name.upper()}] Saved to DB: SSIM={metrics.get('ssim_actual', 0.0):.4f}, "
    ...
    def run(self):
        ...
        try:
            ...
            # Database-only session tracking (no file logging)
            ...
            # Select perturbation severity level(s)
