...
class VLMBatchOrchestrator:
    ...
    def _process_single_question(self, 
        ...
        # Check if this question/image is known to be corrupted
        img_path = question_data.get('image_path', '')
        if is_question_corrupted(data['question_id']) or should_skip_image(img_path, engine):
            log_corrupted_image_skip(img_path, engine, data['question_id'])
            
            # Create error result for corrupted image
        ...
        try:
            ...
            if res == "CUDA_ERROR_HANDLED":
                ...
                res = "ERROR: CUDA device-side assert - input data may be corrupted"
        except Exception as e:
            print(f"⚠️  VLM client error for question {data['question_id']}: {e}")
            res = f"ERROR: VLM client exception - {str(e)}"
        finally:
        ...
        try:
            ...
            with tqdm(total=total_questions, desc=f"   Batch {batch_index + 1}/{total_batches}", leave=False) as pbar:
                for img_path, data_items in batch:
                    ...
                    # Filter out corrupted questions first
                    valid_data_items = []
