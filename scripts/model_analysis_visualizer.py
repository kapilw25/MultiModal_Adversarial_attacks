...
class VLMDataAnalyzer:
    ...
    def plot_attack_generation_efficiency_analysis(self):
        ...
        # """Create comprehensive attack generation efficiency vs effectiveness analysis."""
        # attack_data = self.load_attack_comprehensive_data()
        # if attack_data is None or attack_data.empty:
        #     print("⚠️ No attack comprehensive analysis data found - skipping generation efficiency plot")
        #     return
        
        # plt.figure(figsize=(16, 10))
        
        # # Use database variables directly - no derived variables
        # # X-axis: avg_attack_time (database variable)
        # # Y-axis: abs(avg_accuracy_change) (database variable)
        
        # # Create enhanced color mapping
        # enhanced_attack_colors = {
        #     "White-Box": "#FF6B35",  # Orange
        #     "Black-Box": "#004E89",  # Dark Blue
        # }
        
        # # Apply cluster-aware scaling to BOTH axes using global functions
        # x_original = attack_data['avg_attack_time'].values
        # y_original = abs(attack_data['avg_accuracy_change']).values
        
        # # Transform both axes with cluster-aware scaling
        ...
        # # Plot with both axes cluster-aware scaled
        ...
        #                         s=cat_data['avg_mean_perturbation'] * 20,
        #                         alpha=0.7, color=enhanced_attack_colors.get(category, 'gray'),
        #                         label=f'{category} Attacks', edgecolors='black', linewidth=1.5)
        
        
        # # Add labels using transformed coordinates
        ...
        #                          edgecolor=enhanced_attack_colors.get(row['attack_category'], 'black'), linewidth=1),
        #                 color=enhanced_attack_colors.get(row['attack_category'], 'black'))
        
        # # AUTOMATIC axis ticks using original values
        ...
        # plt.xticks(x_transformed_ticks, [f'{orig:.3f}' for orig in x_auto_ticks])
        ...
        # # Research-standard quadrant analysis with transformed medians
        # median_attack_time_orig = attack_data['avg_attack_time'].median()
        # median_accuracy_change_orig = abs(attack_data['avg_accuracy_change']).median()
        # median_attack_time_trans = x_transform_func(median_attack_time_orig)
        # median_accuracy_change_trans = y_transform_func(median_accuracy_change_orig)
        
        # # Clean reference lines
        # plt.axhline(y=median_accuracy_change_trans, color='gray', linestyle='-', alpha=0.3, linewidth=0.8)
        # plt.axvline(x=median_attack_time_trans, color='gray', linestyle='-', alpha=0.3, linewidth=0.8)
        
        # # Research-appropriate quadrant labels (minimal, professional)
        # plt.text(0.98, 0.98, 'Fast Generation\nHigh Accuracy Change', 
        #         transform=plt.gca().transAxes, fontsize=8, 
        #         ha='right', va='top', alpha=0.7, style='italic')
        
        # plt.text(0.02, 0.98, 'Slow Generation\nHigh Accuracy Change', 
        #         transform=plt.gca().transAxes, fontsize=8,
        #         ha='left', va='top', alpha=0.7, style='italic')
        
        # plt.text(0.98, 0.02, 'Fast Generation\nLow Accuracy Change', 
        #         transform=plt.gca().transAxes, fontsize=8,
        #         ha='right', va='bottom', alpha=0.7, style='italic')
        
        # plt.text(0.02, 0.02, 'Slow Generation\nLow Accuracy Change', 
        #         transform=plt.gca().transAxes, fontsize=8,
        #         ha='left', va='bottom', alpha=0.7, style='italic')
        
        # # Customize plot
        # plt.xlabel('Average Attack Time (seconds)', fontsize=14)
        # plt.ylabel('Average Accuracy Change (%)', fontsize=14)
        # plt.title('Average Attack Time vs Average Accuracy Change\n' + 
        #          'Bubble size indicates Average Mean Perturbation', fontsize=16)
        
        # # Enhanced legend
        # legend_elements = []
        # for category, color in enhanced_attack_colors.items():
        #     legend_elements.append(plt.scatter([], [], color=color, s=100, alpha=0.7, 
        #                                      edgecolors='black', linewidth=1.5, label=f'{category} Attacks'))
        
        # # Add perturbation size legend
        # perturbation_sizes = [5, 15, 25]
        # perturbation_labels = ['Low Perturbation', 'Medium Perturbation', 'High Perturbation']
        # for size, label in zip(perturbation_sizes, perturbation_labels):
        #     legend_elements.append(plt.scatter([], [], s=size*20, alpha=0.7, color='gray', 
        ...
        # plt.savefig(os.path.join(PLOT_DIR, '14_attack_generation_efficiency_analysis.png'), 
        #            dpi=300, bbox_inches='tight')
        # plt.close()
        # print("✅ Created: 14_attack_generation_efficiency_analysis.png")
    ...
    def plot_size_memory_quality_bubble(self, df):
        ...
        return
        # """Create bubble chart using database variables with cluster-aware scaling."""
        ...
        # # Create scatter plot with transformed coordinates - uniform bubble size
        # for i, (_, row) in enumerate(model_data.iterrows()):
        #     family = row['family_name']
        #     plt.scatter(x_transformed[i], y_transformed[i],
        #                s=120,  # Uniform size since cache_hit_ratio values are nearly identical
        #                alpha=0.6,
        ...
        # plt.title('Model Size vs Average GPU Memory Peak\n(Cache Hit Ratio: ~0.999 for all models)', 
        #           fontsize=16, pad=100)
